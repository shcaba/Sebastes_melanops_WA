...
\#create\_management\_table( \# management\_dir = ``C:/Users/Jason.Cope/Documents/Github/Sebastes\_melanops\_WA/Document/tables/tables/'', \#doc\_dir = doc\_dir, \#doc\_names = c(``nca'', `sca'), \#years = 2012:2022, \#table\_names = c(``copper\_ca\_north.csv'', ``copper\_ca\_south.csv''), \#prefix = ``es'')

\#```

\#```\{r, results = `asis'\} \#yrs = hist = 2013:endyr \#nfleets = 2 \#catch = dead = est.dead = input.catch = 0 \#for (i in 1:nfleets)\{ \# name = paste0(``retain(B):\_``,i) \# input.catch = model\(timeseries[model\)timeseries\$Yr \%in\% yrs, name{]} \# catch = cbind(catch, input.catch)

\# name = paste0(``dead(B):\_``,i) \# est.dead = model\(timeseries[model\)timeseries\(Yr %in% yrs, name]  # dead = cbind(dead, est.dead) #} #total.catch <- round(apply(catch, 1, sum),1) #total.dead <- round(apply(dead, 1, sum), 1) #man = read.csv("C:/Users/Jason.Cope/Documents/Github/Sebastes_melanops_WA/Document/tables/tables/brf_OFL_ABC_ACL.csv") #man = man[man
\)Year \%in\% yrs, {]} \#out = cbind(man, total.catch, total.dead)
